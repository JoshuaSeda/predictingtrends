...
def inventory():
    ...
    if request.method == "POST":
        ...
        # FIX: Read all new item fields from form
...
def delete_sale(source, idx):
    ...
    if source == "archive":
        ...
        # Note: archive view is reversed, so adjust index
...
def forecast():
    """
    Rolling 7-day bar chart + rolling 30-day ARIMA prediction window.
    Unlike sales.html (which resets on Monday), this page ALWAYS uses the
    true last N calendar days ending today — regardless of day of week.
    """
    if "user" not in session: return redirect(url_for("login"))

    live_sales     = load_json(SALES_FILE)
    archived_sales = load_json(ARCHIVE_FILE)
    # Combine archive + live so we always have the full rolling history
    all_sales      = archived_sales + live_sales

    today           = date.today()
    seven_days_ago  = today - timedelta(days=6)   # 7 days inclusive of today
    thirty_days_ago = today - timedelta(days=29)  # 30 days inclusive of today

    categories_list = ["Jackets", "T-shirts", "Shorts"]

    # Rolling 7-day window for the bar chart (today - 6 days → today)
    daily_data = {}
    for i in range(7):
        day = seven_days_ago + timedelta(days=i)
        daily_data[day.strftime("%Y-%m-%d")] = {cat: 0 for cat in categories_list}

    # Rolling 30-day window for ARIMA training (more history = better predictions)
    prediction_data = {}
    for i in range(30):
        day = thirty_days_ago + timedelta(days=i)
        prediction_data[day.strftime("%Y-%m-%d")] = {cat: 0 for cat in categories_list}
    ...
    for sale in all_sales:
        ...
        if category in categories_list:
            ...
            if key in prediction_data:
                prediction_data[key][category] += qty

    # Human-readable date range label for the bar chart subtitle
    date_range_label = "{} – {}".format(
        seven_days_ago.strftime("%b %d"),
